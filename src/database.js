...
function storePremapFormFeedback(option, callback) {
	// let newKey = null;
	const premapFeedbackPath = "/feedback/premap/";
	//get current option value
	firebase
		.database()
		.ref(premapFeedbackPath + option)
		.once("value")
		.then(snapshot => {
			let oldOptionCount = snapshot.val();
			console.log("optionVal=", oldOptionCount);
			//get a unique random key for the new point
			// newKey = firebase
			//   .database()
			//   .ref(premapFeedbackPath)
			//   .push().key;

			//store marker data at the unique location
			firebase
				.database()
				.ref(premapFeedbackPath) //feedback + "/email/" + email)
				.update({ [option]: oldOptionCount + 1 });
